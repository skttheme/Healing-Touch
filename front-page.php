...
<?php if( get_theme_mod( 'hide_boxes' ) == '') { ?>
  <div class="feature-box-main site-aligner">
    ...
    <?php wp_reset_query(); ?>
    <?php } else { ?>
    <div class="feature-box <?php if($f%4==0){ ?>last<?php } ?>"> <img class="iconset" src="<?php echo esc_url(get_template_directory_uri()); ?>/images/icon<?php echo esc_html($f); ?>.png" />
      <div class="feature-title">
        <?php esc_html_e('Page Title','healing-touch'); ?>
        <?php echo esc_html($f); ?></div>
      <!-- feature-title -->
